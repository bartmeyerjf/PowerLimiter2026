// ------------------------------------------------------
// PWM Reading Library via ISR + FreeRTOS
// ------------------------------------------------------
...
void pwmProcessingTask(void *pvParameters);
...
void pwmProcessingTask(void *pvParameters) {
  for (;;) {
    ...
    if (safePeriod > 0 && safeHighTime <= safePeriod) {
      ...
      float dutyCycle = ((float)safeHighTime / (float)safePeriod) * 100.0;
...
void setupPWM() {
  ...
  xTaskCreate(
    pwmProcessingTask,   // Name of the function that contains the task
    ...
    10,                  // Task priority (High priority)
